...
if (exists("fit_brms_mmp")) {
    p3_in_m <- apply(posterior_epred(fit_brms_mmp, re_formula = NA), 2, median)
...
best_mmp_t <- gbm.perf(brt_mmp_fit, method = "cv", plot.it = FALSE)
...
grid_brms_mmp <- data.frame(
    MaxDHW_s = seq(dhw_min_s, dhw_max_s, length.out = 100),
    secc3m_s = 0, cloudp_90_s = 0, prop_acropora_s = 0, depth = median(d_mmp$depth, na.rm = T)
)
if (exists("fit_brms_mmp")) {
...
## ----tri-dataset-comparison-summary-table-------------------------------------
...
if (file.exists(cache_mmp_beta)) {
    ...
    fit_brms_mmp <- brm(
        ...
        ), # stronger regularization for small N
        chains = 4, iter = 4000, warmup = 2000, cores = 4, seed = 42,
...
cat("\nBeta R²:")
print(bayes_R2(fit_brms_mmp))
cat("\nBinomial-OLRE R²:")
print(bayes_R2(fit_brmsb_mmp))
...
# Panel B: DHW x Prop Acropora Interaction (Acropora on X axis)
al_range_raw <- seq(quantile(d$prop_acropora, 0.05, na.rm = TRUE), quantile(d$prop_acropora, 0.95, na.rm = TRUE), length.out = 100)
al_range_s <- (al_range_raw - sp$prop_acropora["mean"]) / sp$prop_acropora["sd2"]
dhw_levels <- c(4, 8, 12)
dhw_s <- (dhw_levels - sp$MaxDHW.mean["mean"]) / sp$MaxDHW.mean["sd2"]

gb <- expand.grid(prop_acropora_s = al_range_s, MaxDHW_s = dhw_s) %>%
    mutate(secc3m_s = 0, cloudp_90_s = 0, depth = median(d$depth, na.rm = T))
gb <- brms_pred_mmp(m, gb) %>%
    mutate(
        prop_acropora = prop_acropora_s * sp$prop_acropora["sd2"] + sp$prop_acropora["mean"],
        lbl = factor(paste0(round(MaxDHW.mean, 0), " DHW"), levels = paste0(dhw_levels, " DHW"))
    )
pB3 <- ggplot(gb, aes(x = prop_acropora * 100, y = pred, colour = lbl, fill = lbl)) +
    geom_ribbon(aes(ymin = pred_lo, ymax = pred_hi), alpha = 0.12, colour = NA) +
    geom_line(linewidth = 1) +
    scale_colour_brewer(palette = "Set1") +
    scale_fill_brewer(palette = "Set1") +
    labs(x = "% Acropora Cover", y = "Mortality Proportion", colour = "DHW Level", fill = "DHW Level", title = "(B) DHW x Prop. Acropora Interaction") +
    coord_cartesian(ylim = c(0, 1)) +
    theme_bw(base_size = 11) +
    theme(legend.position = c(0.35, 0.85))
...
pC3 <- ggplot(gc, aes(x = MaxDHW.mean, y = pred, colour = lbl, fill = lbl)) +
    es +
    el +
    geom_ribbon(aes(ymin = pred_lo, ymax = pred_hi), alpha = 0.12, colour = NA) +
    geom_line(linewidth = 1) +
...
cl_q <- quantile(d$cloudp_90, c(0.1, 0.5, 0.9), na.rm = TRUE)
cl_qs <- (cl_q - sp$cloudp_90["mean"]) / sp$cloudp_90["sd2"]
gc <- expand.grid(MaxDHW_s = seq(dhw_min_s, dhw_max_s, length.out = 200), cloudp_90_s = cl_qs) %>%
    mutate(secc3m_s = 0, prop_acropora_s = 0, depth = median(d$depth, na.rm = T))
...
# Panel D: Worst vs Best
cl_10s <- (quantile(d$cloudp_90, 0.10, na.rm = T) - sp$cloudp_90["mean"]) / sp$cloudp_90["sd2"]
cl_95s <- (quantile(d$cloudp_90, 0.95, na.rm = T) - sp$cloudp_90["mean"]) / sp$cloudp_90["sd2"]
a_10s <- (quantile(d$prop_acropora, 0.10, na.rm = T) - sp$prop_acropora["mean"]) / sp$prop_acropora["sd2"]
a_90s <- (quantile(d$prop_acropora, 0.90, na.rm = T) - sp$prop_acropora["mean"]) / sp$prop_acropora["sd2"]
gd <- data.frame(
    MaxDHW_s = rep(seq(dhw_min_s, dhw_max_s, length.out = 200), 2),
    scenario = rep(c("Worst case", "Best case"), each = 200),
    cloudp_90_s = rep(c(cl_10s, cl_95s), each = 200),
    prop_acropora_s = rep(c(a_90s, a_10s), each = 200),
    secc3m_s = 0, depth = median(d$depth, na.rm = T)
)
gd <- brms_pred_mmp_olre(m, gd)
gd$scenario <- factor(gd$scenario, levels = c("Worst case", "Best case"))
ge <- gd %>%
    select(MaxDHW.mean, scenario, pred, pred_lo, pred_hi) %>%
    pivot_wider(id_cols = MaxDHW.mean, names_from = scenario, values_from = c(pred, pred_lo, pred_hi))
...
ga <- data.frame(
    ...
    cloudp_90 = median(d$cloudp_90, na.rm = T), histmDHW6 = median(d$histmDHW6, na.rm = T),
    winyear_sd = median(d$winyear_sd, na.rm = T), mcur_90 = median(d$mcur_90, na.rm = T),
    winyear_mean = median(d$winyear_mean, na.rm = T), yrsince6 = median(d$yrsince6, na.rm = T),
...
pB3 <- ggplot(gb, aes(x = MaxDHW.mean, y = pred, colour = lbl, fill = lbl)) +
    es +
    el +
    geom_ribbon(aes(ymin = pred_lo, ymax = pred_hi), alpha = 0.12, colour = NA) +
    geom_line(linewidth = 1) +
