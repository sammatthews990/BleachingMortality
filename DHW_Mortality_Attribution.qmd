...
d_m <- dat.ml.mant.clean %>% select(any_of(cols_comb)) %>% mutate(Dataset = "Manta Tow")
d_l <- dat.ml.ltmp.clean %>% select(any_of(cols_comb)) %>% mutate(Dataset = "LTMP Benthic")
d_p <- dat.ml.mmp.clean  %>% select(any_of(cols_comb)) %>% mutate(Dataset = "MMP Inshore")
...
if (file.exists(cache_comb_beta)) {
    ...
    p3_in_c <- apply(posterior_epred(fit_brms_comb, re_formula = NA), 2, median)
...
f_brt_c <- Mort.prop ~ MaxDHW.mean + secc3m + winyear_mean + histmDHW6 + mcur_90 + winyear_sd + cloudp_90 + yrsince6 + Dataset
brt_comb_fit <- gbm(
...
predictor_vars_c <- c("MaxDHW.mean", "secc3m", "winyear_mean", "histmDHW6", "mcur_90", "winyear_sd", "cloudp_90", "yrsince6", "Dataset")
labels_map_c <- c(
    MaxDHW.mean = "Max DHW (°C-weeks)",
    secc3m = "Secchi Depth (m)",
    winyear_mean = "Winter SST Mean (°C)",
...
predictor_vars_c_env <- c("MaxDHW.mean", "secc3m", "winyear_mean", "histmDHW6", "mcur_90", "winyear_sd", "cloudp_90", "yrsince6")
...
f_str_c <- Mort.prop ~ MaxDHW.mean + secc3m + winyear_mean + histmDHW6 + mcur_90 + winyear_sd + cloudp_90 + yrsince6 + Dataset
...
coef_comb <- coef_comb %>%
    ...
    mutate(
        ...
        term_clean = case_when(
            ...
            term == "mcur_90" ~ "Current Speed (90th)",
            term == "winyear_sd" ~ "Winter SST SD (°C)",
            term == "cloudp_90" ~ "Cloud Cover (90th)",
...
if (exists("fit_brms_comb")) {
    ...
    p3_zarr_in <- apply(posterior_epred(fit_brms_comb, newdata = grid_brms_zarr_eval, re_formula = NA), 2, median)
...
labels_map_z <- c(
    ...
    mcur_90_zarr = "Current Speed (90th)",
    winyear_sd_zarr = "Winter SST SD (°C)",
    cloudp_90_zarr = "Cloud Cover (90th)",
...
pred_z_env <- c("MaxDHW_zarr", "secc3m_zarr", "winyear_mean_zarr", "histmDHW6_zarr", "mcur_90_zarr", "winyear_sd_zarr", "cloudp_90_zarr", "yrsince6_zarr")
...
(pA_z_comp + pB_z_comp) / (pC_z_comp + pD_z_comp)
```
...
# Ensure prop_acropora is initialized if not present
if (!"prop_acropora" %in% names(dat.ml.expanded)) {
    if (exists("dat.ml.comb.clean") && "prop_acropora" %in% names(dat.ml.comb.clean)) {
        dat.ml.expanded$prop_acropora <- median(dat.ml.comb.clean$prop_acropora, na.rm = TRUE)
    } else {
...
if (exists("fit_brms_comb")) {
    ...
    p3_exp_in <- apply(posterior_epred(fit_brms_comb, newdata = grid_brms_exp_eval, re_formula = NA), 2, median)
...
pred_exp_env <- c("MaxDHW_val", "secc3m", "winyear_mean", "histmDHW6", "mcur_90", "winyear_sd", "cloudp_90", "yrsince6")
...
p_exp_hist <- ggplot(grid_exp_hist, aes(x = MaxDHW_val, y = histmDHW6, z = pred)) +
    geom_tile(aes(fill = pred)) +
    geom_contour(colour = "white", alpha = 0.4, linewidth = 0.3) +
    scale_fill_viridis_c(option = "magma", name = "Predicted\nMortality") +
    labs(x = "Max DHW (°C-weeks)", y = "Prior Exposure (histmDHW6)", title = "(C) DHW × Prior Bleaching", subtitle = "Thermal Stress × Acclimation/Vulnerability") +
    theme_bw(base_size = 11)

p_exp_sec + p_exp_winm + p_exp_hist + plot_layout(guides = "collect")
```
