...
To evaluate non-linear environmental thresholds, complex multi-way interaction structures, and predictor contributions without imposing parametric functional assumptions, we fit a **Boosted Regression Tree (BRT)** model (`gbm`) using the exact same set of 6 environmental predictors (`MaxDHW.mean`, `secc3m`, `cloudp_90`, `histmDHW6`, `mcur_90`, `winyear_sd`) on the clean dataset ($N=110$).

The BRT model was optimized using 5-fold cross-validation, achieving an optimal boosting tree iteration of $T_{\text{opt}} = 579$. To quantify sampling and structural uncertainty around the non-linear marginal response curves, we performed **$B = 100$ bootstrap iterations** (resampling with replacement), refitting the BRT and generating 90% and 95% confidence intervals across evaluation grids for every environmental predictor.
...
```{r brt-pdp-boot}
#| label: brt-pdp-boot
#| fig-cap: "Boosted Regression Trees (BRT) — Partial dependence plots with 90% and 95% bootstrapped confidence intervals (B = 100)."
#| fig-width: 10
#| fig-height: 7
...
p_comp <- ggplot() +
    ...
    geom_ribbon(data = pdp_dhw, aes(x = x, ymin = lo_90, ymax = hi_90), fill = "#008080", alpha = 0.3) +
...
p_comp +
    ...
    coord_cartesian(ylim = c(0, 1)) +
    theme_bw(base_size = 12) +
    theme(legend.position = c(0.28, 0.85), panel.grid.minor = element_blank())
```
