...
if (!all(file.exists(validation_files))) {
    stop('Validation files are missing. Run Rscript scripts/build_validation_splits.R first.')
}
...
if (!all(file.exists(model_result_files))) {
    stop('Candidate-model results are missing. Run Rscript scripts/fit_candidate_models.R first.')
}
